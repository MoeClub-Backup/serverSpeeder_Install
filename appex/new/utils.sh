...
showStats() {
    ...
    while [ $count -lt $TOTAL_TIME ]; do
        ...
        while [ $eNum -lt $CPUNUM ]; do
            ...
            eval $(cat /proc/net/appex$e/stats | egrep '(NumOf.*Flows)|(Bytes)|(Packets)|(NfBypass)' | sed 's/\s*//g')
            ...
            [ -z "$WanInPackets" ] && WanInPackets=0
            [ -z "$LanInPackets" ] && LanInPackets=0
            [ -z "$WanOutPackets" ] && WanOutPackets=0
            [ -z "$LanOutPackets" ] && LanOutPackets=0
            ...
            eval wanInPacketsPre=\$wan_in_Packets_pre_$e
            eval lanInPacketsPre=\$lan_in_Packets_pre_$e
            eval wanOutPacketsPre=\$wan_out_Packets_pre_$e
            eval lanOutPacketsPre=\$lan_out_Packets_pre_$e
            
            wanIn=$WanInBytes
            lanIn=$LanInBytes
            wanOut=$WanOutBytes
            lanOut=$LanOutBytes
            wanInPackets=$WanInPackets
            lanInPackets=$LanInPackets
            wanOutPackets=$WanOutPackets
            lanOutPackets=$LanOutPackets
            ...
            [ -z "$wanInPre" ] && {
                ...
                wanOutPre=$wanOut
                lanOutPre=$lanOut
                wanInPacketsPre=$wanInPackets
                lanInPacketsPre=$lanInPackets
                wanOutPacketsPre=$wanOutPackets
                lanOutPacketsPre=$lanOutPackets
            }
            ...
            eval wan_in_Packets_pre_$e=$wanInPackets
            eval lan_in_Packets_pre_$e=$lanInPackets
            eval wan_out_Packets_pre_$e=$wanOutPackets
            eval lan_out_Packets_pre_$e=$lanOutPackets
            
            #calc ratio
            wanInRate=$(echo "($wanIn - $wanInPre) / (1024 * $CALC_ITV)" | bc -l)
            ...
            cmd="$cmd printf \"\${showLan:+wan }in :  $HL_START%.2F$HL_END kB/s\n\${showLan:+wan }out: $HL_START%.2F$HL_END kB/s\npackets : $HL_START%.2F$HL_END\n\" $wanInRate $wanOutRate $wanPacketsRate;"
            [ -n "$showLan" ] && cmd="$cmd printf \"lan out: $HL_START%.2F$HL_END kB/s\nlan in :  $HL_START%.2F$HL_END kB/s\nlan packets : $HL_START%.2F$HL_END\n\" $lanOutRate $lanInRate $lanPacketsRate;"
            ...
            cmd="$cmd printf \"active tcp sessions: $HL_START%d$HL_END\n\" $NumOfActFlows;"
